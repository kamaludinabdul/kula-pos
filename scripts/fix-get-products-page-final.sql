-- Definitive Fix for get_products_page RPC
-- This script clears all previous versions and creates a single standardized version.

-- 1. Drop all known variations to avoid "function is not unique" errors (400 Bad Request)
...
-- 2. Create the standardized version
-- Using UUID for p_store_id for consistency with the schema
...
SECURITY DEFINER -- Crucial! Allows the function to bypass RLS and filter by p_store_id internally
LANGUAGE plpgsql
AS $$
DECLARE
    v_offset INT;
...
BEGIN
    ...
    -- 3. Return the payload that DataContext expects
