-- Update get_products_page RPC to include stock_type filter
-- Drop previous versions to avoid conflicts
DROP FUNCTION IF EXISTS get_products_page(UUID, INT, INT, TEXT, TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS get_products_page(UUID, INT, INT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
...
CREATE OR REPLACE FUNCTION get_products_page(
    ...
    p_stock_type TEXT DEFAULT 'all', -- 'all', 'Barang', 'Jasa'
...
-- Force PostgREST to reload the schema cache so the new 9-parameter RPC is recognized immediately
