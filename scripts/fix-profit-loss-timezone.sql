-- FIX: get_profit_loss_report RPC Timezone Bug
-- Removes ::DATE casts that shift timezone boundaries
...
BEGIN
    ...
    WHERE cf.store_id = p_store_id
      -- FIX: Remove ::DATE cast to respect timezone from frontend
...
SELECT 'Berhasil mengupdate get_profit_loss_report (Fix Timezone Bug OPEX)!' as status;
