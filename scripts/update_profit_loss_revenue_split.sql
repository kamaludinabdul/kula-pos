-- UPDATE: get_profit_loss_report RPC
-- Adds revenue split calculation explicitly for Barang and Jasa

-- 1. Drop the existing functions (both UUID and TEXT versions to be safe)
...
-- 2. Create the updated version with TEXT parameter for better SDK compatibility
...
BEGIN
    ...
    WHERE store_id::TEXT = p_store_id
      ...
      AND date <= p_end_date
      -- Liberal status matching for now to identify if this is the cause
      AND (
    ...
    WITH expanded_items AS (
        ...
        LEFT JOIN products pr ON item->>'id' ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' AND pr.id = (item->>'id')::UUID
    ...
    SELECT 
        COALESCE(SUM(q), 0), 
        COALESCE(SUM(q * c), 0),
        -- Item revenue is calculated as qty * (price - discount)
    ...
    RETURN jsonb_build_object(
        ...
        -- DEBUG FIELDS
        'debug_total_transactions_all', (SELECT count(*) FROM transactions WHERE store_id::TEXT = p_store_id),
        'debug_store_id', p_store_id,
        'debug_start_date', p_start_date,
        'debug_end_date', p_end_date
    );
END;
$$;
